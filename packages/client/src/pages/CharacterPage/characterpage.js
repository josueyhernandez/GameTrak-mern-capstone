    ...
    async function selectCharacter() {
      ...
      function changeCharacter(e) {
        ...
        if (e.target.value === "none") {
          ...
          axios.get(`/api/chars/character/${e.target.value}`).then((res) => {
            setCurrentChar({
              name: res.data.name,
      ...
      async function obtainAttrInfo(attrs) {
        ...
        if (attrs !== undefined) {
          attrs.map(async (attributeId) => {
            await axios.get(`/api/attr/${attributeId}`).then((res) => {
              console.log(res.data);
      ...
      async function changeAttr(e) {
        ...
        setEditMode(false);
        await axios
          .put("/api/chars/change-attribute", {
            character: currentChar.id,
            attribute: selectedAttr.info._id,
            newValue: editVal,
          })
          .then((res) => console.log(res.data));
        axios.get(`/api/chars/character/${currentChar.id}`).then((res) => {
          obtainAttrInfo(res.data.attributes);
        });
      }
    }
        ...
        {validated ? (
          <div className="main">
            ...
            <div className="log">
              ...
              <Button
                className="back"
                onClick={() => window.location.replace("/games")}
              >
                Back to List
              </Button>
              <div id="current-game">
                {state.currentGame && (
                  <div id="game-title">{state.currentGame.name}</div>
                )}
                {state.currentGame && (
                  <img height="50px" src={state.currentGame.image} />
                )}
              </div>
            </div>
            <div className="header">
              <h3>
                <a className="header-item" href="/character">
                  Character
                </a>
              </h3>
              <h3>
                <a className="header-item" href="/item">
                  Items
                </a>
              </h3>
              <h3>
                <a className="header-item" href="/places">
                  Places
                </a>
              </h3>
              <h3>
                <a className="header-item" href="/skills">
                  Skills
                </a>
              </h3>
              <h3>
                <a className="header-item" href="/attributes">
                  Attributes
                </a>
              </h3>
            </div>
            ...
            <div>
              {state.currentGame && (
                <h1 class="game-title">{state.currentGame.name} Characters</h1>
              )}
            </div>
            <div class="characterId">
              <div class="character-info">
                ...
                <p>
                  Name: <span class="name">{currentChar.name}</span>
                </p>
                ...
                <Form id="edit-att">
                  ...
                  {selectedAttr?.type === 1 && (
                    <Form.Control
                      ...
                      type="text"
                      placeholder={`Edit ${selectedAttr?.info?.name} Here`}
                    />
                  )}
                  {selectedAttr?.type === 2 && (
                    <Form.Control
